...
try:
    with sqlite3.connect("Venta_cosmeticos.db") as conn:
        c = conn.cursor()
        c.execute("CREATE TABLE IF NOT EXISTS venta (clave INTEGER PRIMARY KEY, descripcion TEXT NOT NULL, piezas INTEGER NOT NULL, precio TEXT NOT NULL, fecha_venta DATE);")
        ...
        def cargar():
            ...
            while True:
                ...
                if descripcion == "":
                    ...
                    valores = {"clave": clave, "nombre":descripcion, "unidades": unidades_venta, "precio": precio, "fecha": fecha_vta}
                    c.execute("INSERT INTO venta VALUES(:clave, :nombre, :unidades, :precio, :fecha)", valores)
        ...
        def consulta():
            ...
            if rtotales:
                ...
                c.execute("SELECT clave, descripcion, piezas, precio, fecha_venta FROM venta WHERE clave = :clave_b",clave_buscar)
